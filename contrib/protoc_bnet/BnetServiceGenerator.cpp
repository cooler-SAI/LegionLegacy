...
{
    printer->Print(vars_,
        "class $classname$ : public ServiceBase\n"
        "{\n"
        " public:\n"
        "\n"
    ...
    for (int i = 0; i < descriptor_->method_count(); i++)
    {
        ...
        if (method->output_type()->name() != "NO_RESPONSE")
        {
            printer->Print(sub_vars,
                ...
                "  sLog->outDebug\(LOG_FILTER_PROTOBUF, \"%s Server called client method $full_name$($input_type_name${ %s })\",\n"
...
void BnetServiceGenerator::GenerateServerCallMethod(pb::io::Printer* printer)
{
    ...
    fprintf(stderr, "GenerateServerCallMethod %i\n", descriptor_->method_count());

    for (int i = 0; i < descriptor_->method_count(); i++)
    {
        fprintf(stderr, "GenerateServerCallMethod method_id %i\n", Battlenet::method_id);
        ...
        printer->Print(sub_vars,
            ...
            "        sLog->outDebug\(LOG_FILTER_PROTOBUF, \"%s Failed to parse request for $full_name$ server method call.\", GetCallerInfo().c_str());\n"
        ...
        if (method->output_type()->name() != "NO_RESPONSE")
        {
            printer->Print(sub_vars,
                ...
                "      sLog->outDebug\(LOG_FILTER_PROTOBUF, \"%s Client called server method $full_name$($input_type_name${ %s }) returned $output_type_name${ %s } status %u.\",\n"
        ...
        else
        {
            printer->Print(sub_vars,
                ...
                "      sLog->outDebug\(LOG_FILTER_PROTOBUF, \"%s Client called server method $full_name$($input_type_name${ %s }) status %u.\",\n"
    ...
    printer->Print(vars_,
        "    default:\n"
        "      sLog->outError\(LOG_FILTER_PROTOBUF, \"Bad method id %u.\", methodId);\n"
    ...
    for (int i = 0; i < descriptor_->method_count(); i++)
    {
        ...
        if (method->output_type()->name() != "NO_RESPONSE")
        {
            printer->Print(sub_vars, "uint32 $classname$::Handle$name$($input_type$ const* request, $output_type$* response) {\n"
                "  sLog->outError\(LOG_FILTER_PROTOBUF, \"%s Client tried to call not implemented method $full_name$({ %s })\",\n"
                "    GetCallerInfo().c_str(), request->ShortDebugString().c_str());\n"
