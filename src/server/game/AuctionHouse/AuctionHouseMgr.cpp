...
bool AuctionEntry::LoadFromDB(Field* fields)
{
    ...
    if (!auctioneerInfo)
        TC_LOG_ERROR(LOG_FILTER_SERVER_LOADING, "Auction %u has not a existing auctioneer Entry: %u", Id, auctioneer);
    else
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_SERVER_LOADING, "Auction %u has auctioneer Entry: %u with wrong faction %u", Id, auctioneer, factionTemplateId);
        return false;
    }
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_SERVER_LOADING, "Auction %u has not a existing item : %u", Id, itemGUIDLow);
        return false;
    }
    return true;
}
