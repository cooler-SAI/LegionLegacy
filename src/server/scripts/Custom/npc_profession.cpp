...
class npc_profession : public CreatureScript
{
    ...
    {
        player->ADD_GOSSIP_ITEM(9, "Список профессий", GOSSIP_SENDER_MAIN, 196);
        player->ADD_GOSSIP_ITEM(7, "Прощайте", GOSSIP_SENDER_MAIN, 220);
    ...
    void CompleteLearnProfession(Player* player, Creature* creature, SkillType skill)
    {
        if (!player->GetFreePrimaryProfessionPoints() && !IsSecondarySkill(skill))
            creature->MonsterWhisper("Вы уже изучили две профессии!", player->GetGUID());
        else
        {
            if (!LearnAllRecipesInProfession(player, skill))
                creature->MonsterWhisper("Внутренняя ошибка!", player->GetGUID());
        }
    }
    ...
    bool OnGossipSelect(Player* player, Creature* _creature, uint32 /*uiSender*/, uint32 uiAction) override
    {
        ...
        switch (uiAction)
        {
            ...
            case 196:
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TRAINER, "Алхимия", GOSSIP_SENDER_MAIN, 1);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TRAINER, "Кузнечное дело", GOSSIP_SENDER_MAIN, 2);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TRAINER, "Кожевничество", GOSSIP_SENDER_MAIN, 3);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TRAINER, "Портняжное дело", GOSSIP_SENDER_MAIN, 4);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TRAINER, "Инженерное дело", GOSSIP_SENDER_MAIN, 5);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TRAINER, "Наложение чар", GOSSIP_SENDER_MAIN, 6);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TRAINER, "Ювелирное дело", GOSSIP_SENDER_MAIN, 7);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TRAINER, "Начертание", GOSSIP_SENDER_MAIN, 8);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TRAINER, "Кулинария", GOSSIP_SENDER_MAIN, 9);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TRAINER, "Первая помощь", GOSSIP_SENDER_MAIN, 10);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TRAINER, "Травничество", GOSSIP_SENDER_MAIN, 11);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TRAINER, "Снятие шкур", GOSSIP_SENDER_MAIN, 12);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TRAINER, "Горное дело", GOSSIP_SENDER_MAIN, 13);
                player->ADD_GOSSIP_ITEM(4, "Вернуться в главное меню!", GOSSIP_SENDER_MAIN, 200);
                player->PlayerTalkClass->SendGossipMenu(20011, _creature->GetGUID());
                break;
            case 1:
                if (player->HasSkill(SKILL_ALCHEMY))
                {
                    _creature->MonsterWhisper("Вы уже освоили эту профессию!", player->GetGUID());
                    player->PlayerTalkClass->SendCloseGossip();
                    break;
                }
