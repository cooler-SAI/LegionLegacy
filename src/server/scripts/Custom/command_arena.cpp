        ...
        // 2c2
        {
            ...
            if (teamResult && teamResult->GetRowCount())
            {
                ...
                for (auto& member : memberList)
                {
                    ...
                    sLog->outArenaSeason("За %u место в топе 2на2 получил %s(%u) token=%u, achiv=(%s), title=(%s), item(%s), побед %u, игр %u, рейтинг %u %s",
                ...
                sLog->outArenaSeason("Недостаточно участников %s", debugOnly ? "debugOnly" : "");
        }
        ...
        // 3c3
        {
            ...
            if (teamResult && teamResult->GetRowCount())
            {
                ...
                for (auto& member : memberList)
                {
                    ...
                    sLog->outArenaSeason("За %u место в топе 3на3 получил %s(%u) token=%u, achiv=(%s), title=(%s), item(%s), побед %u, игр %u, рейтинг %u %s",
                    member.second.teamNumber, member.second.name.c_str(), member.first, member.second.tokenCount, achivStr.str().c_str(), titleStr.str().c_str(), itemStr.str().c_str(), member.second.wins, member.second.games, member.second.rating, debugOnly ? "debugOnly" : "");
                }
            }
            else
