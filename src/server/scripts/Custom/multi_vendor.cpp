            ...
            if (type == DONATE_TYPE_ITEM)  // = GOSSIP_SENDER_MAIN
            {
                ...
                else
                {
                        ...
                        {          
                            for (auto itr = donvencat->additionals.begin(); itr != donvencat->additionals.end(); ++itr)
                            {
                                ...
                                if (player->HasTitle(titleInfo))
                                    sprintf(printinfo, "%s |cff00ff00[%s]|r", printinfo, (loc_idx == 8 ? "Куплено" : "Buyed")); 
                    ...
                    if (donvencat->next_page != -1)
                        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, (loc_idx == 8 ? ">> Следующая страница" : ">> Next page"), GOSSIP_SENDER_MAIN, donvencat->next_page);

                    if (donvencat->prev_page != -1)
                        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, (loc_idx == 8 ? "<< Предыдущая страница" : "<< Previous page"), GOSSIP_SENDER_MAIN, donvencat->prev_page);

                    if (action != 0)
                        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, (loc_idx == 8 ? "Назад" : "Back"), GOSSIP_SENDER_MAIN, donvencat->parent);
...
class npc_transmogrify : public CreatureScript
{
    public:
        ...
        void TransmogrifyItem(Player* player, Creature* creature)
        {
            ...
            {
                handler.PSendSysMessage("Использовать трансмогрификацию через данного моба можно только на рисовках!");
                return;
            }
        ...
        void TransmogrifyItemLeft(Player* player, Creature* creature)
        {
            ...
            if (transmogrified->GetChildItem().IsEmpty()) 
            {
                handler.PSendSysMessage("Для данного предмета не существует 'левой' части");
                return;
            }
